...
fun Activity.mostrarDialogo(titulo: String, mensaje: String, aceptar:(DialogInterface)->Unit, cancelar:(DialogInterface)->Unit){ //Le paso como parametro el DialogInterface para poder cerrar luego el dialog con it
    val builder: AlertDialog.Builder= AlertDialog.Builder(this)
    builder
        .setTitle(titulo)
        .setMessage(mensaje)
        .setCancelable(false)  //Aunque se haga click fuera del dialogo, no desaparece, es decir, se tiene que pulsar en aceptar o cancelar antes
        .setPositiveButton("Aceptar"){dialog,which->aceptar(dialog)}
        .setNegativeButton("Cancelar"){dialog,which->cancelar(dialog)}
